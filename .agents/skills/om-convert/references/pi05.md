...
- `verify` is mandatory for a complete conversion and must use the canonical versioned observation
  batch, not generated dummy inputs. The current orchestrator does not forward its `batch_path` to the
  verifier, so update `pi05_export` to pass `--batch-path`, task, seed, schedule, and user limits to
  `verify_pi05_split_equivalence`. Extend the verifier to write machine-readable max-abs, mean-L1, and
  cosine metrics and evaluate the user limits instead of its fixed cosine heuristic. Add regression
  tests before running the long conversion. Extend the `pi05_export` CLI/profile schema with explicit
  verifier seed, max-abs, mean-L1, cosine, and metrics-output parameters. The lower-level `convert_om`
  defaults to `--precision_mode_v2=origin`; use it for the conservative baseline. After accuracy passes,
  an ATC-default candidate may use explicit `--precision_mode_v2=fp16` against the same target package.
  Do not pass the literal value `default` or add an unvalidated generic `--atc-arg` path. Extend and test
  top-level forwarding or call the lower-level converter separately. The templates below assume the verifier
  flags have been implemented; use their final names consistently and do not run them against the current API
  first.
...
python3 -m model_utils.pi05_export \
    ...
    --task "RESOLVED_TASK" \
    --verify-seed "RESOLVED_SEED" \
    --verify-max-abs-max "RESOLVED_ONNX_MAX_ABS" \
    --verify-mean-l1-max "RESOLVED_ONNX_MEAN_L1" \
    --verify-cosine-min "RESOLVED_ONNX_COSINE" \
    --verify-metrics-json "/absolute/path/to/pi05_export_run/reports/torch-vs-onnx.json" \
...
python3 -m model_utils.pi05_export \
    ...
    --verify-mean-l1-max "RESOLVED_ONNX_MEAN_L1" \
    --verify-cosine-min "RESOLVED_ONNX_COSINE" \
    --verify-metrics-json "/absolute/path/to/pi05_export_run-fastgelu/reports/torch-vs-onnx.json" \
    --soc-version "RESOLVED_SOC_VERSION" \
    --device "RESOLVED_EXPORT_DEVICE" \
    --dtype fp16 \
    --deployment "RESOLVED_DEPLOYMENT" \
...
python3 -m model_utils.pi05_export \
    ...
    --verify-mean-l1-max "RESOLVED_ONNX_MEAN_L1" \
    --verify-cosine-min "RESOLVED_ONNX_COSINE" \
    --verify-metrics-json "RESOLVED_ONNX_METRICS_JSON" \
    --steps vlm_onnx,ae_onnx,verify,vlm_om,ae_om
```
...
or runtime adaptation errors. Apply max-abs, mean-L1, and cosine limits to Torch vs ONNX, and the
current `loss_compare` mean-L1 and cosine limits to Torch vs OM, as defined by `accuracy.md`. FastGELU
may trade accuracy for speed, but the tradeoff must be measured rather than assumed.
