...
- `verify` is mandatory for a complete conversion and must use the canonical versioned observation
  ...
  already defaults to `--precision_mode_v2=origin`; do not add a top-level `--atc-arg` for the default
  path. If the user explicitly selects `default`, extend and validate top-level forwarding or call the
  lower-level converter separately. The templates below assume the verifier flags have been
  implemented; use their final names consistently and do not run them against the current API first.
