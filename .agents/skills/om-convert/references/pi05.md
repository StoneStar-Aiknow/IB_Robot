...
### 1. Confirm PaliGemma Assets

Always ask how to resolve the PaliGemma tokenizer/assets before asking about pipeline steps. Present
these choices:
...
- ask the user to approve a bundle-local destination, defaulting to `<model_path>/tokenizer`;
...
### 4. Confirm Denoising Schedule

Ask this after PaliGemma assets, steps, and FastGELU. Explain that new Action Expert OM files predict velocity and the
