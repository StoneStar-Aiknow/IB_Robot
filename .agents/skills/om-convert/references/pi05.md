...
and keeps the Action Expert in FP16. The bundled `pi05-ae-attn-mlp-sq-v1` applies SmoothQuant (`alpha=0.5`,
`epsilon=1e-5`) to all 18 Action Expert layers' q/k/v/o, fused MLP, and down projections: 108 W8A8 nodes in
total. It requires exact NPUGeglu plus complete 10-step AE calibration trajectories and keeps the VLM in FP16.
Recalibrate and repeat accuracy/performance validation for every new checkpoint; never reuse another
checkpoint's SmoothQuant plan, W8A8 ONNX, scales, or OM.
