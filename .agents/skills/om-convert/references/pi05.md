...
| Validation | Strict manifest loading, Torch-vs-ONNX, Torch-vs-OM, and per-OM ais_bench loop 50 |
...
- `verify` is mandatory for a complete conversion and must use the canonical versioned observation
  ...
  verifier seed, max-abs, mean-L1, cosine, and metrics-output parameters. The templates below assume
  those implemented flags; use their actual final names consistently.
...
`ais_bench --loop 50` according to `benchmark.md`, even in report-only mode or when a threshold is not
