...
3. **依赖 / setup / build 变更的 Verification 强制门禁**：修改 `package.xml` 依赖声明或全局 setup/build 流程文件时，PR 描述中的双平台 Docker Verification 必填；其 `Verified commit` 必须与 PR 最新 `head.sha` 一致。提取脚本会将缺失或不一致生成为阻塞检查；review 默认只检查声明，不自动执行验证。
...
- 进行 IB_Robot PR review 时，必须先处理 `.pr.mandatory_review_checks`，重点检查
  ...
  Verification 是否覆盖双平台，以及 `Verified commit` 是否与 `head_sha` 一致
