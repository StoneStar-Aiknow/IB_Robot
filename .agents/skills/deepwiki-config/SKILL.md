...
- 验证结果：`id_to_label`、`title_to_label`、`hierarchy` 三者一致，或列出不一致项
- 提示用户：label 通过去除括号内容 + slug 去虚词自动生成；更新场景下已有标签会被保留，如需调整可手动编辑 `doc_config.json`
