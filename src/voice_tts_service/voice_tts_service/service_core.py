...
from voice_tts_service.backend import TTSBackend
...
class TTSServiceCore:
    """Serialize one backend through an explicit caller-owned lock."""

    def __init__(self, backend: TTSBackend | None, limits: TTSLimits) -> None:
    ...
    def synthesize_prepared(self, prepared: PreparedSynthesisRequest) -> SynthesisOutput:
        ...
        if self.backend is None:
            raise TTSError("MODEL_NOT_READY", "Voice TTS model backend is not ready")
        ...
        for index, segment_text_value in enumerate(prepared.segments):
            ...
            result = self.backend.synthesize(segment_text_value, prepared.prompt, prepared.prompt_text)
            wav_data, duration_sec = float_pcm_to_wav(result.samples, result.sample_rate)
