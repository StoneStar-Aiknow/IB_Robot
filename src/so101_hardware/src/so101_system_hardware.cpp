...
int SafeSMSSTS::read_with_timeout(unsigned char *data, int length,
                                  unsigned long timeout_ms) {
...
SO101SystemHardware::on_init(const hardware_interface::HardwareInfo &info) {
  ...
  try {
    ...
    RCLCPP_WARN(rclcpp::get_logger("SO101SystemHardware"),
...
SO101SystemHardware::on_activate(const rclcpp_lifecycle::State &) {
  ...
  const auto cleanup_activation = [this, &cleanup_done]() {
    if (cleanup_done) {
      return;
    }
    if (!detail::disable_torque_on_abort(motor_ids_, [this](u8 id, u8 enabled) {
  ...
  for (size_t i = 0; i < motor_ids_.size(); i++) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("SO101SystemHardware"),
                 "Motor ID %d found.", id);
  }
  ...
  for (size_t i = 0; i < motor_ids_.size(); i++) {
    ...
    if (!require_ack(id, "disable torque",
    ...
    if (!require_ack(id, "unlock EPROM",
    ...
    if (!require_ack(id, "enable torque",
  ...
  // frame.
  sms_sts_.syncReadBegin(motor_ids_.size(), FEEDBACK_READ_LEN,
  ...
  // Initial sync
  if (sms_sts_.syncReadPacketTx(motor_ids_.data(), motor_ids_.size(),
...
SO101SystemHardware::read(const rclcpp::Time &time, const rclcpp::Duration &) {
  ...
  int read_len =
      sms_sts_.syncReadPacketTx(motor_ids_.data(), motor_ids_.size(),
                                FEEDBACK_START_ADDR, FEEDBACK_READ_LEN);
  if (read_len <= 0) {
    RCLCPP_WARN_THROTTLE(rclcpp::get_logger("SO101SystemHardware"),
...
SO101SystemHardware::write(const rclcpp::Time &, const rclcpp::Duration &) {
  ...
  sms_sts_.SyncWritePosEx(motor_ids_.data(), motor_ids_.size(),
