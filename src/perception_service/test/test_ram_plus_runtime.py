...
from perception_service.ram_plus_adapter import RAM_PLUS_POSTPROCESSING, RAM_PLUS_PREPROCESSING, RAMPlusAdapter
...
def _result(logits) -> NamedTensorResult:
    values = np.full((1, 4585), -80.0, dtype=np.float32)
    values[0, : len(logits)] = logits
