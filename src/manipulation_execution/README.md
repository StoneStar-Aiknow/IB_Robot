...
本包也是抓取行为的唯一实现源。监督式真机测试、批量实验和 Hermes 都必须通过
`robot-skill execute pick_object` 进入 Capability Gateway；`/manipulation/execute_pick` 是 Gateway
准入后的内部 delegated action，调用方不得自行构造 binding 或 executor identity。

旧监督式执行器、direct action client 及其 legacy-only replay benchmark 已删除。所有行为测试直接覆盖正式
phases 和纯几何模块；`test_pick_single_source.py` 会在 CI 中确保不会重新引入绕过 Gateway 的抓取入口。
...
| 接口 | 类型 | 说明 |
| --- | --- | --- |
| `/embodied/execute_skill` | `ibrobot_msgs/action/SkillCommand` | 对外抓取入口；使用 `pick_object` 和 `target_name` |
...
`target_query`、`timeout_sec` 和 `expected_executor`。`target_query` 是 Grounded-SAM2/GraspGen 的文本查询
