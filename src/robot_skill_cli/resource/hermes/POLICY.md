...
- 只读感知数据只能通过 `ibrobot-perceive --topic <t> --field <f>` 包装命令读取；不得直接调用
  `ros2 topic echo`、`ros2 topic list`、`ros2 param get` 或任何其他 `ros2` 子命令。`ibrobot-perceive`
  的 topic/field 白名单硬编码在源码中，不得通过修改 config.yaml 扩展。
- `ibrobot-perceive` 通过 `ros2 topic echo --once` 读取下一条消息，返回的是单次点时值，不是持久快照。
  对 `/voice/speech_direction` 这类事件型 topic，只有在发布方活跃时才能在超时内取到值；返回值在消费时
